...
type App struct {
	ctx        context.Context
	dataFolder string
	// mu serializes the API handlers: each one is a read-modify-write cycle on
	// a single JSON file, so concurrent requests would otherwise lose edits.
	mu sync.Mutex
}
